  ...
  return (
    <main style={{ background: "var(--color-dark)", minHeight: "100vh" }}>
      ...
      >
        <Link
          href="/"
          className="flex items-center gap-1 text-white font-bold text-lg hover:opacity-80 transition-opacity"
          style={{ fontFamily: "var(--font-heading)" }}
        >
          NJ<span className="w-1.5 h-1.5 rounded-full inline-block ml-0.5" style={{ background: "var(--color-yellow)" }} />
        </Link>
        <Link
          href="/"
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        {/* Header */}
        <div className="mb-12">
          <p className="text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2" style={{ color: "var(--color-yellow)" }}>
            <span className="w-6 h-0.5 bg-current" /> Legal
          </p>
          <h1
        ...
        >
          <p className="text-sm leading-relaxed mb-4" style={{ color: "rgba(255,255,255,0.6)" }}>
            NickJain.org (&ldquo;Website,&rdquo; &ldquo;we,&rdquo; &ldquo;us,&rdquo; or &ldquo;our&rdquo;) respects your privacy and is committed to protecting your personal information. This Privacy Policy explains how we collect, use, disclose, store, and safeguard your information when you use this website or engage with our recruiting and professional services.
          </p>
          <p className="text-sm font-semibold mb-3" style={{ color: "rgba(255,255,255,0.45)" }}>This Policy applies when you:</p>
          <ul className="space-y-1.5 mb-4">
            {["Visit NickJain.org","Submit resumes or professional inquiries","Communicate via email, phone, or SMS/text","Request consulting or recruiting information","Use website forms or communication tools","Interact with professional networking or business services"].map((item) => (
              <li key={item} className="flex items-start gap-2.5 text-sm" style={{ color: "rgba(255,255,255,0.5)" }}>
                <span className="mt-2 w-1 h-1 rounded-full shrink-0" style={{ background: "var(--color-yellow)" }} />
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="mb-12">
          <a
            href="mailto:privacy@nickjain.org"
            ...
            >
              ...
              {s.contact ? (
                <div className="space-y-1 text-sm" style={{ color: "rgba(255,255,255,0.55)" }}>
                  <p className="font-bold text-white">NickJain.org</p>
                  <p>Email: <a href="mailto:privacy@nickjain.org" className="underline hover:text-white" style={{ color: "rgba(255,255,255,0.55)" }}>privacy@nickjain.org</a></p>
                </div>
              ) : (
                <>
                  {s.body && <p className="text-sm mb-3 leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>{s.body}</p>}
                  {s.list && (
                  ...
                  {s.extra && <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>{s.extra}</p>}
                </>
              )}
            </div>
          ))}
